...
function linuxkit_build() {
	...
	cat "linuxkit-templates/${kernel_info['TEMPLATE']}.template.yaml" |
		HOOK_KERNEL_IMAGE="${kernel_oci_image}" HOOK_KERNEL_ID="${inventory_id}" HOOK_KERNEL_VERSION="${kernel_oci_version}" \
			HOOK_CONTAINER_CGV1_BOOTKIT_IMAGE="${HOOK_CONTAINER_CGV1_BOOTKIT_IMAGE}" \
			HOOK_CONTAINER_CGV1_DOCKER_IMAGE="${HOOK_CONTAINER_CGV1_DOCKER_IMAGE}" \
			HOOK_CONTAINER_CGV1_MDEV_IMAGE="${HOOK_CONTAINER_CGV1_MDEV_IMAGE}" \
			...
			envsubst '$HOOK_VERSION $HOOK_KERNEL_IMAGE $HOOK_KERNEL_ID $HOOK_KERNEL_VERSION $HOOK_CONTAINER_CGV1_BOOTKIT_IMAGE $HOOK_CONTAINER_CGV1_DOCKER_IMAGE $HOOK_CONTAINER_CGV1_MDEV_IMAGE $HOOK_CONTAINER_BOOTKIT_IMAGE $HOOK_CONTAINER_DOCKER_IMAGE $HOOK_CONTAINER_MDEV_IMAGE $HOOK_CONTAINER_CONTAINERD_IMAGE $HOOK_CONTAINER_RUNC_IMAGE' \
