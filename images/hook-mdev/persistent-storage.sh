...
if [ -n "${serial}" ] && [ -n "${model}" ]; then
	...
	if [ -n "${wwid_raw}" ]; then
		...
		wwid_model=$(echo "${wwid_model}" | sed 's/\\0//g')                         # Remove all instances of literal backslash-zero "\0" (not really nulls, thanks, Dell!)
		...
		wwid_serial=$(echo "${wwid_serial}" | sed 's/\\0//g')                       # Remove all instances of literal backslash-zero "\0" (not really nulls, thanks, Dell!)
